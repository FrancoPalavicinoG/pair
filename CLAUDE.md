...
- **Código nuevo de aprendizaje, no lo escribas tú.** Desde P2 (auth) en adelante, el código de cada feature nueva lo escribe el usuario en su editor. Antes de que empiece, explica el concepto y las alternativas con su trade-off; si hace falta, da un ejemplo aislado, no el código final de esa pieza. Si el usuario se traba o tiene un error, ayuda a debuggear guiando el razonamiento (qué mirar, qué probar), sin resolverlo directo. Revisa el código una vez escrito. No aplica a scaffolding mecánico (carpetas, configs) ni a código de fases ya cerradas.
