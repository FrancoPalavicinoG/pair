...
- **Barra de rango con objetivo** (variante de la anterior, para Training load/foco de carga): a diferencia de la barra de arriba (tramos categóricos fijos de una rampa), acá el objetivo es un rango numérico propio por métrica (`targetMin`-`targetMax`, confirmado por Garmin) sobre una escala `[0, max(targetMax, valor) * 1.05]` — una banda resaltada (`--zone3` verde sólido, sin opacidad — mismo tono exacto que el gauge de zona; probado también al 40%/70% pero leía lavado comparado con el gauge, decisión final de Franco fue el sólido. El marcador, mismo `--zone3` cuando el valor entra en el rango, se distingue de la banda por sobresalir 2px arriba/abajo del alto de la banda, no por contraste de color — a revisar si se pierde legibilidad en pantallas de densidad normal) marca ese rango, y el marcador cae en el valor real: verde si el valor entra en el rango, naranjo (`--zone2`, mismo hue de "atención" que el resto del sistema) si queda afuera, por encima o por debajo. Tres de estas conviven en una sola tile chica (Anaerobic/Aerobic hi/Aerobic lo) — label + barra + valor en **una sola línea** por métrica (no label arriba y barra abajo, como la barra de rango de VFC): a la tile mínima (180px) no le entran las dos líneas × 3 métricas junto con el ACWR que ya mostraba el widget. Labels abreviados ("Aerobic hi"/"Aerobic lo") para que quepan en una columna fija de `w-16` sin truncar. Implementación: `apps/web/src/app/(app)/dashboard/_components/widgets/training-load.tsx`.
...
- **Mini-barras diarias**: una barra corta por día (7 días, sin ejes), para una magnitud donde cada día ya es una unidad comparable por sí sola (ej. horas de entrenamiento, minutos en zona) — no una tendencia continua. Mismo hue de identidad que el resto del gráfico. Regla para elegir entre esto y el sparkline (`apps/web/src/lib/sparkline.ts`): si la métrica no tiene "unidad de día" fuerte (ej. resting HR, que es un valor que fluctúa) usa sparkline; si cada día es su propia unidad de conteo (ej. horas entrenadas ese día) usa mini-barras.
  - **Variante con label de día** (Training hours/HIIT/Running, `weekly-bar-chart.tsx`): un label de una letra debajo de cada barra (lunes->domingo), altura normalizada al máximo de esos 7 días (no a un techo fijo entre widgets ni entre semanas). Días futuros de la semana en curso (todavía no pasaron) llevan un placeholder de borde punteado sin relleno, nunca una barra en 0 — un día real sin entreno y un día que no llegó todavía no pueden leerse igual. El label del día de hoy pasa a `--ink` (mismo tono que el valor grande de la tile) en vez de `--graphite`, más `font-medium` (500, el peso más alto que define JetBrains Mono en este proyecto — no hay un 700/bold real para esta fuente utilitaria), para que se note de un vistazo en qué punto de la semana está parado el gráfico.
