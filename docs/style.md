...
- **Barra de fases de sueño**: timeline horizontal segmentado, mismo principio que "barra apilada horizontal" (gap de 2px entre segmentos, sobre `--panel`) pero **categórico por fase, no ordinal por magnitud** — el ancho de cada segmento es proporción de tiempo, no una escala de "cuánto". Color: rampa `--sleep1`…`--sleep4` (ver Sistema de color), en orden despierto → ligero → REM → profundo.
