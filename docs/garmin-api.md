...
| Métricas | sueño diario (duración total ya la tenemos; **fases del sueño, sin confirmar**) | GET | por confirmar | El resumen diario trae `sleepingSeconds`, `measurableAwakeDuration`, `measurableAsleepDuration` (totales), pero no desglose de fases (profundo/ligero/REM) — esa forma de dato vive en otro endpoint, sin confirmar todavía |
| Métricas | HRV (estado) | GET | por confirmar | No aparece en el resumen diario — endpoint separado, sin confirmar |
