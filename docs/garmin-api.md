...
| Métricas | `calendarDate` es por día-calendario **local**, no UTC | — | **confirmado** (2026-09-01, con cuenta real) | Nuestro código calculaba "hoy" con `new Date().toISOString().slice(0,10)` (UTC) tanto al sincronizar como al leer widgets. Un sync corrido a las 01:22 UTC del 2 de septiembre (todavía 1 de septiembre para el usuario) pidió `calendarDate=2026-09-02` — un día que localmente no había empezado — y Garmin devolvió el resumen vacío para esa fecha, mientras los datos reales de "hoy" estaban bajo `2026-09-01`. Se agregó `users.timezone` (IANA) + `packages/core/src/date.ts: localDateString` para calcular "hoy" en la zona del usuario en vez de UTC, tanto en `syncDailyMetrics` como en `findTodayMetrics`/`findRecentDailyMetrics`/`findWeeklySummary` |
...
| Métricas | sueño diario, score + fases | GET | **confirmado** (2026-08-27) | `GET /sleep-service/sleep/dailySleepData?date=YYYY-MM-DD` — **probado y es idéntico** a la otra ruta candidata (`wellness-service/wellness/dailySleepData/{displayName}?...`), así que usamos esta (no necesita `displayName` en la URL). `deepSleepSeconds`/`lightSleepSeconds`/`remSleepSeconds`/`awakeSleepSeconds` = el desglose de fases. `sleepScores.overall.value: 75` = exacto el número que muestra la web como "Puntuación de sueño". El timeline tipo hypnograma sale de `sleepMovement` (un punto por minuto, `{startGMT, endGMT, activityLevel}`) — no es literalmente fase por fase, es nivel de movimiento; para pintar algo tipo el hypnograma de la referencia hay que derivarlo de esto, Garmin no da un array de "fase por bloque horario" directo. Fixture anonimizada: `docs/fixtures/sleep-daily.anon.json`. **Hallazgo 2026-08-31, con cuenta real**: `dailySleepDTO.sleepTimeSeconds` de este endpoint **no coincide** con `sleepingSeconds` del resumen diario (`usersummary-service`, fila de arriba) — para el mismo día real se vieron valores distintos (26570 vs 27240) y, para el día en curso, `usersummary.sleepingSeconds` puede estar en `0` mientras este endpoint ya tiene el dato completo de esa noche (`sleepTimeSeconds: 36780`, con `remSleepSeconds`/`deepSleepSeconds`/`lightSleepSeconds`/`awakeSleepSeconds` sumando exacto ese total) — `usersummary.sleepingSeconds` parece ser un valor parcial/no siempre actualizado, no la fuente confiable. `packages/sync` guardaba `sleepSeconds` desde `usersummary.sleepingSeconds`; se corrigió para leer `sleepDto.sleepTimeSeconds`, consistente con el resto de los campos de sueño que ya usaban este endpoint. Filas ya sincronizadas antes de este fix quedan con el valor viejo (no se hizo backfill retroactivo). |
...
| Métricas | predisposición para entrenar (readiness) | GET | **confirmado** (2026-08-27, corregido 2026-09-02) | `GET /metrics-service/metrics/trainingreadiness/{YYYY-MM-DD}`, devuelve una lista con **una entrada por recálculo del día** (`inputContext`: `AFTER_WAKEUP_RESET`, `UPDATE_REALTIME_VARIABLES`, `AFTER_POST_EXERCISE_RESET`…), no una sola. **El valor "actual" que muestra la web/app es el de mayor `timestampLocal`, no el de `AFTER_WAKEUP_RESET`** — asumir la entrada de la mañana (como hacía `packages/sync` hasta este fix) deja el readiness pegado al valor de al despertar todo el día, aunque haya recálculos más nuevos. `level`/`score` con el desglose completo por factor (`sleepScoreFactorPercent`, `hrvFactorPercent`, `acwrFactorPercent`, `recoveryTimeFactorPercent`, `stressHistoryFactorPercent`, cada uno con su `*FactorFeedback` en texto). Fixture anonimizada: `docs/fixtures/training-readiness.anon.json` |
