...
- Diseño real de las vistas de detalle por métrica (`/dashboard/metrics/[key]`): spec futuro, separado, a pedido tuyo. (Pasada mínima ya hecha, ver nota de cierre más abajo: la vista dejó de ser el widget cuadrado metido en una columna angosta, pero todavía muestra la misma info que la tile — el desglose real sigue pendiente.)
...
## Notas de cierre (curación MVP)

- **Catálogo reducido a 10 widgets fijos** para el MVP, paso previo a diseñar en serio `/dashboard/metrics/[key]` (diferido más arriba "a pedido tuyo"): `recent_activity`, `weekly_hours`, `body_battery`, `endurance_score`, `vo2_max_running`, `sleep_phases`, `hrv`, `training_status`, `training_load`, `readiness`.
- **Los widgets dinámicos `weekly_distance:<sport>` también se curaron para el MVP**: `MVP_SPORT_WIDGETS` (`registry.ts`) los limita a `running` y `hiit`, aunque el usuario haya entrenado otro deporte esa semana — reintroduce a propósito el hardcode de deportes que este spec evitaba más arriba ("no una lista fija de deportes hardcodeada"), mismo criterio de curación temporal que los widgets fijos. Ampliar el set a futuro es agregar al `Set`, no un cambio estructural.
- Los otros 12 (`steps`, `resting_hr`, `sleep`, `spo2`, `respiration`, `hill_score`, `vo2_max_cycling`, `altitude_acclimation`, `weight`, `bmi`, `stress`, `heat_acclimation`) **no se borraron**: se sacaron de `FIXED_WIDGET_REGISTRY` (`registry.ts`) pero su código sigue intacto en `daily-metrics.tsx`, `stress.tsx` y `heat-acclimation.tsx`, listos para reactivarse en una próxima iteración sin reescribir nada.
- Un usuario con alguna de esas keys ya guardada como visible en `dashboard_layouts` no ve nada roto: `getEffectiveLayout` la ignora sola vía el mismo filtro contra `knownKeys` que ya manejaba las keys v1.
- **Se sacó `MAX_VISIBLE_WIDGETS` (tope de 15 + el aviso "X/15 widgets visible" en `/dashboard/widgets`)**: con el catálogo curado el universo total es como mucho 12, nunca llega a 15 — el tope había quedado en código muerto (la rama que lo dispara ya no es alcanzable) y el aviso confundía más de lo que ayudaba, sugería margen que no existía. Vuelve cuando el catálogo crezca de nuevo en una próxima iteración.
- **`weekly_distance:<sport>` sin distancia GPS** (ej. HIIT): en vez de mostrar "0 m" (dato sin valor), la tile muestra el tiempo entrenado esa semana (`bucket.thisWeek.durationSeconds`, ya lo traía `findWeeklySummary` pero solo agregado globalmente — se sumó también por deporte en `WeeklySportBucket`) con el mismo patrón de delta `%` semana-a-semana que ya usa `weekly_hours`.
- **`vo2_max_running` no comparte la ventana genérica de `renderMetric`** (`daily-metrics.tsx`, 14 días / "vs yesterday"): como el VO2 Max casi no varía día a día, usa una ventana de 30 días y compara contra el punto más viejo de esa ventana ("vs last month"), vía las nuevas opciones de `renderMetric` (`historyDays`, `compareLabel`, `compareToRangeStart`, `noDataLabel`) — con default que reproduce el comportamiento de siempre, así que `body_battery`/`endurance_score` (los otros consumidores) no cambiaron.
  - **Hallazgo probando en vivo**: el punto de comparación no puede ser ciegamente `series[0]` (el primer día de la ventana) — `daily_metrics` tiene una fila por día para varias métricas a la vez, así que el primer día de una ventana de 30 puede tener `vo2_max_running` en `null` si esa métrica empezó a registrarse a mitad de la ventana (visto con una cuenta real: VO2 Max solo tenía 13 días de historia dentro de la ventana de 30). `compareToRangeStart` busca la fila más vieja de la serie que sí tenga el dato (`series.slice(0, -1).find(row => row[key] != null)`), no la primera fila a secas — si ninguna lo tiene, cae a `noDataLabel`.
- **Pasada mínima de `/dashboard/metrics/[key]`** (sin diseño nuevo, misma info que ya mostraba la tile — el desglose real por métrica sigue diferido "a pedido tuyo" más arriba):
  - `StatTile` ya tenía un modo no-cuadrado (`square={false}`, sin usar hasta ahora); `TileShell` (`stat-tile.tsx`) ganó el mismo modo, mismo criterio: fondo/hover/label idénticos, sin `aspect-square`/`overflow-hidden`.
  - Cada `renderX` de `_components/widgets/*.tsx` recibe un segundo (o tercer, en `renderWeeklyDistance`) parámetro `square = true` que reenvía a su `StatTile`/`TileShell` — la grilla de `/dashboard` sigue pasando `true` explícito, `/dashboard/metrics/[key]` pasa `false`. `WidgetEntry.render` (`registry.ts`) documenta la firma `(userId, square?)` para que un widget nuevo la siga.
  - `/dashboard/metrics/[key]/page.tsx` dejó el `max-w-md` — mismo fix de layout de escritorio que `/dashboard/widgets` (`docs/style.md`, "Layout de escritorio").
  - **"Most recent activity" ahora prioriza `activity.name`** sobre el deporte (mismo criterio que `/activities`) y su tile en `/dashboard` linkea directo a `/activities/<garminActivityId>` (la ficha real de esa actividad), no a `/dashboard/metrics/recent_activity`. `WidgetEntry` ganó un `href?` opcional (`registry.ts`) para este único caso; el resto de los widgets sigue con el link genérico a `/dashboard/metrics/[key]`, calculado una sola vez en `dashboard/page.tsx` y pasado como prop (`WidgetItem.href`) en vez de que `dashboard-layout-editor.tsx` lo arme a mano.
